...
std::array<int, 10> paint_letterboxes(int start, int end)
{
    ...
    for (; start <= end; ++start)
    {
        for (auto& elem : std::to_string(start))  //convert int to string, to iterate over
        {
            ++DigitFreq[((int)elem -48)];           //convert char to string then ascii to decimal
        }
    }
    return DigitFreq;
}
